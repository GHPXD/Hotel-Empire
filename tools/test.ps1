...
try {
    ...
    $suites = @('foundation_test', 'construction_test', 'simulation_test', 'save_test', 'management_test', 'progression_test')
    if ($Stress) { $suites += @('save_multiseed_test', 'stress_test') }
    if ($Visual) { $suites += @('ui_smoke', 'ui_resume', 'ui_management', 'ui_progression') }
